...
install_docker() {
  ...
  if systemd_available; then
    run_cmd_sudo systemctl enable --now docker
...
_install_docker_apt() {
  ...
  run_cmd_sudo apt-get install -y ca-certificates curl gnupg lsb-release
  ...
  download_file "https://download.docker.com/linux/ubuntu/gpg" "${tmpdir}/docker.gpg"
  run_cmd_sudo gpg --dearmor -o /etc/apt/keyrings/docker.gpg < "${tmpdir}/docker.gpg"
  run_cmd_sudo chmod a+r /etc/apt/keyrings/docker.gpg

  echo "deb [arch=${arch} signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/ubuntu ${codename} stable" \
    | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null

  run_cmd_sudo apt-get update -y
  run_cmd_sudo apt-get install -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin
}

_install_docker_rpm() {
  log_step "Installing Docker on Fedora/RHEL/CentOS..."
  # shellcheck disable=SC2086
  run_cmd sudo $PKG_REMOVE docker docker-client docker-client-latest docker-common \
    docker-latest docker-latest-logrotate docker-logrotate docker-engine 2>/dev/null || true
  # shellcheck disable=SC2086
  run_cmd sudo $PKG_INSTALL dnf-plugins-core 2>/dev/null || true

  if [[ "$OS_ID" == "fedora" ]]; then
    run_cmd_sudo dnf config-manager --add-repo https://download.docker.com/linux/fedora/docker-ce.repo
  else
    run_cmd_sudo dnf config-manager --add-repo https://download.docker.com/linux/centos/docker-ce.repo
  fi
  # shellcheck disable=SC2086
  run_cmd sudo $PKG_INSTALL docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin
}

_install_docker_arch() {
  log_step "Installing Docker on Arch/Manjaro..."
  run_cmd_sudo pacman -S --noconfirm docker docker-compose
}

_install_docker_zypper() {
  log_step "Installing Docker on openSUSE..."
  run_cmd_sudo zypper install -y docker docker-compose
}

_install_docker_alpine() {
  log_step "Installing Docker on Alpine..."
  run_cmd_sudo apk add docker docker-compose
  run_cmd_sudo rc-update add docker boot
  run_cmd_sudo service docker start
}

_install_docker_script() {
  log_step "Using Docker convenience script..."
  local tmpdir; tmpdir="$(make_tmpdir)"
  download_file "https://get.docker.com" "${tmpdir}/get-docker.sh"
  run_cmd_sudo sh "${tmpdir}/get-docker.sh"
}
...
add_user_docker_group() {
  ...
  if ! group_exists "docker"; then
    ...
    run_cmd_sudo groupadd docker
  fi

  run_cmd_sudo usermod -aG docker "$target_user"
